...
description: >
  ...
  "create a masterplan", "prepare my build docs", or "help me start building with Lovable".
  Also use when students in the Agent Builder Academy need to assemble their project documentation.
  This skill covers Steps 1–3 of the 9-step fast track workflow.
---

# Lovable PRD Builder (Steps 1–3)

Generate a complete, Lovable.dev-optimised Product Requirements Document in four parts — plus a condensed Knowledge Base summary — ready for immediate use in Lovable's Knowledge Base settings.

## Why Documentation-First Building Works

Lovable is a powerful AI coding agent — but it has no persistent memory. Every session starts from scratch. Without documentation, AI forgets your vision after a few prompts, each session starts over, features drift from original intent, and you waste credits on corrections.

**The solution:** PRD documents + a Knowledge Base configuration = your AI's long-term memory.

When you complete this skill, you'll have:
- 4 structured documents that Lovable reads before every action
- A condensed Knowledge Base summary to paste into Lovable's settings
- Consistent vision across every build session
- Features that align with documented specifications
...
Read `references/lovable-platform.md` to understand Lovable.dev's tech stack, constraints, Four Ways to Build, and how the Knowledge Base system works. This context shapes every document you'll produce.

Also read `references/design-toolbox.md` for design inspiration sources — have these ready when the user asks about visual direction.
...
- Any apps or websites whose aesthetic you admire? (If unsure, suggest browsing references/design-toolbox.md)
...
The Implementation Plan translates the Masterplan into a build sequence optimised for Lovable's prompting workflow. Key principles:

- **One prompt, one task.** Each step should be a single, focused instruction that Lovable can execute without ambiguity.
- **Layout before behaviour.** Get the visual structure right first, then add interactivity. Fixing layout after behaviour is painful in Lovable.
...
- **Suggest design references** from `references/design-toolbox.md` if the user wants inspiration for components or visual style.
...
4. **Brief the user.** Explain which document goes where and what comes next:
   - `lovable-knowledge-base.md` → Paste into Lovable's Knowledge Base settings
   - The four full documents → Upload to `/docs` folder in Lovable
   - The Implementation Plan → Use as your prompt-by-prompt build sequence
   - **Next step**: Use the `lovable-build-guide` skill (or `/lovable-guide` command) for Steps 4–9 — executing the build inside Lovable

### What the PRD Generation Prompt Looks Like

If the user wants to regenerate PRDs directly inside Lovable (rather than with Claude), provide them with Prompt 1 from `references/prompts-library.md`. This is a standalone prompt they can paste into Lovable's chat to generate all four documents from a project description.
...
|------|---------|---------------|
| `masterplan.md` | Strategic foundation | Upload to Lovable `/docs` folder |
| `implementation-plan.md` | Build sequence | Upload to Lovable `/docs` folder |
| `design-guidelines.md` | Visual contract | Upload to Lovable `/docs` folder |
| `app-flow-pages-and-roles.md` | Structural blueprint | Upload to Lovable `/docs` folder |
| `lovable-knowledge-base.md` | Condensed knowledge base | Paste into Lovable → Settings → Knowledge |

## What Comes Next (Steps 4–9)

After delivering the PRD documents, briefly describe the next phase to the user:

> **Your next step is the Build Execution workflow (Steps 4–9):**
> 1. Upload your docs to Lovable's `/docs` folder and use the Central Source of Truth prompt
> 2. Paste the Knowledge Base configuration into Lovable settings (3 minutes)
> 3. Use the Magic Prompt as your repeating build loop
> 4. Maintain docs with `rules.md`, `changelog.md`, and regular KB updates
> 5. Use file tagging and Visual Editor for precision editing
> 6. Deploy with one click when ready
>
> Use the `/lovable-guide` command or the `lovable-build-guide` skill for a full walkthrough of Steps 4–9 with copy-ready prompts.

## Resources

### references/
- `lovable-platform.md` — Lovable.dev tech stack, build modes, AI memory problem, Knowledge Base system, and prompting best practices
...
- `prompts-library.md` — All copy-ready Lovable prompts (PRD generation, KB config, Magic Prompt, maintenance)
- `design-toolbox.md` — Component libraries, design inspiration, AI generation tools, community resources
